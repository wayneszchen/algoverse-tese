...
client = anthropic.Anthropic(api_key="")  # Replace with your actual Anthropic API key
...
def generate_neutral_prompts(topics, num_per_topic):
    """Generate neutral prompts using Anthropic Claude API"""
    all_prompts = []
...
Format: Return only the prompts as a numbered list, one prompt per line."""
        ...
        try:
            response = client.messages.create(
                model="claude-opus-4-20250514",  # Updated to working model
                max_tokens=1500,
                temperature=0.8,
            ...
            prompts_text = response.content[0].text.strip()
    ...
    for topic in topics:
        ...
        for i in range(num_per_topic):
            ...
            try:
                ...
                middle_content = response.content[0].text.strip()
...
def main():
    print("=== Enhanced Multi-Topic Prompt Generator (Anthropic Version) ===\n")
    
    use_api = input("Do you want to use Anthropic API for enhanced prompts? (y/n): ").strip().lower()
    if use_api != "y":
